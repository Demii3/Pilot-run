    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="incomeTypeForm">
          ...
          <div class="mb-3">
            <div class="form-check">
              <input class="form-check-input" type="checkbox" id="incomeTypeRecurring" checked>
              <label class="form-check-label" for="incomeTypeRecurring">Recurring</label>
            </div>
          </div>
        </form>
      </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="deductionTypeForm">
          ...
          <div class="mb-3">
            <div class="form-check">
              <input class="form-check-input" type="checkbox" id="deductionTypeRecurring" checked>
              <label class="form-check-label" for="deductionTypeRecurring">Recurring</label>
            </div>
          </div>
        </form>
      </div>
          ...
          <div class="row g-3">
            ...
            <div class="col-md-2">
              ...
              <div class="form-check income-recurring-check">
                <input class="form-check-input" type="checkbox" id="selectedDeductionRecurring" disabled>
        ...
        <form id="employeeIncomeForm">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-3">
              <label class="form-label">Apply Mode</label>
              <div class="form-check income-recurring-check">
                <input class="form-check-input" type="checkbox" id="selectedIncomeRecurring" disabled>
...
async function loadDeductionTypes() {
  ...
  try {
    ...
    if (rows.length === 0) {
      tableBody.innerHTML = '<tr><td colspan="3" class="text-center">No deduction types found.</td></tr>';
...
function openDeductionTypeForm(reset = true) {
  ...
  if (reset) {
    ...
    document.getElementById('deductionTypeRecurring').checked = true;
  }
...
async function saveDeductionType() {
  ...
  const recurring = document.getElementById('deductionTypeRecurring').checked ? 1 : 0;
  const taxable = 1;
...
async function editDeductionType(id) {
  try {
    ...
    document.getElementById('deductionTypeRecurring').checked = Number(item.recurring) === 1;
...
async function loadIncomeTypes() {
  ...
  try {
    ...
    const rows = result.data || [];
    if (rows.length === 0) {
      tableBody.innerHTML = '<tr><td colspan="5" class="text-center">No income types found.</td></tr>';
...
function openIncomeTypeForm(reset = true) {
  ...
  if (reset) {
    ...
    document.getElementById('incomeTypeRecurring').checked = true;
  }
...
async function saveIncomeType() {
  ...
  const recurring = document.getElementById('incomeTypeRecurring').checked ? 1 : 0;
...
async function editIncomeType(id) {
  try {
    ...
    document.getElementById('incomeTypeRecurring').checked = item.recurring == 1;
  ...
  try {
    ...
    if (assignmentData.success) {
      ...
      employeeDeductionAssignments = assignmentData.data.map(item => {
        ...
        cost: item.cost,
        recurring: latestType ? latestType.recurring : 0,
...
function filterAssignedIncomeByCutoff(assignedIncomeData = [], incomeTypesData = [], dateFrom = null, dateTo = null) {
  ...
  return (assignedIncomeData || []).filter(item => {
    const typeNameKey = String(item.type_of_income || '').trim().toLowerCase();
    const isRecurring = recurringMap[typeNameKey] === 1;
    if (isRecurring) {
      return true;
    }
