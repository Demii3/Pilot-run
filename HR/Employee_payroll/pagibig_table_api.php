...
try {
    ...
    if ($method === 'GET') {
        ...
        if (isset($_GET['draw'])) {
            ...
            $columns = ['id', 'salary_from', 'salary_to', 'contribution_rate', 'maximum_contribution', 'description', 'year'];
            $orderColumnName = isset($columns[$orderColumn]) ? $columns[$orderColumn] : 'salary_from';
            
            // Build WHERE clause
            $whereClause = "`year` = ?";
            if (!empty($searchValue)) {
                $whereClause .= " AND (CAST(`salary_from` AS CHAR) LIKE '%$searchValue%' OR CAST(`salary_to` AS CHAR) LIKE '%$searchValue%' OR CAST(`contribution_rate` AS CHAR) LIKE '%$searchValue%' OR `description` LIKE '%$searchValue%')";
            }
            
            // Get total count
            $countSql = "SELECT COUNT(*) as count FROM `pagibig_table` WHERE `year` = ?";
            ...
            // Get filtered count
            $filteredCountSql = "SELECT COUNT(*) as count FROM `pagibig_table` WHERE $whereClause";
            ...
            // Get data
            $sql = "SELECT * FROM `pagibig_table` WHERE $whereClause ORDER BY `$orderColumnName` $orderDir LIMIT ?, ?";
        ...
        $sql = "INSERT INTO `pagibig_table` (`year`, `salary_from`, `salary_to`, `contribution_rate`, `maximum_contribution`, `description`) 
                VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($dbc, $sql);
        mysqli_stmt_bind_param($stmt, 'idddds', $year, $salaryFrom, $salaryTo, $contributionRate, $maximumContribution, $description);
