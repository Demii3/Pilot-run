...
try {
    ...
    if ($method === 'GET') {
        ...
        if (isset($_GET['draw'])) {
            ...
            $columns = ['id', 'salary_from', 'salary_to', 'contribution_rate', 'maximum_contribution', 'fixed_amount', 'description', 'year'];
            ...
            // Get total count
            $countSql = "SELECT COUNT(*) as count FROM `philhealth_table` WHERE `year` = ?";
            ...
            // Get filtered count
            $filteredCountSql = "SELECT COUNT(*) as count FROM `philhealth_table` WHERE $whereClause";
            ...
            // Get data
            $sql = "SELECT * FROM `philhealth_table` WHERE $whereClause ORDER BY `$orderColumnName` $orderDir LIMIT ?, ?";
